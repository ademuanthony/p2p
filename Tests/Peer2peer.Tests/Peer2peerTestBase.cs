...
namespace Peer2peer.Tests
{
    public abstract class Peer2peerTestBase : AbpIntegratedTestBase<Peer2peerTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected Peer2peerTestBase()
        {
        ...
        protected void UsingDbContext(Action<Peer2PeerDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Peer2PeerDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Peer2PeerDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Peer2PeerDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Peer2PeerDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Peer2PeerDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Peer2PeerDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Peer2PeerDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Peer2PeerDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Peer2PeerDbContext, Task<T>> func)
        {
            T result;
